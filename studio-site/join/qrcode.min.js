 ...
 *  http://www.d-project.com/ — QR Code is a registered trademark of DENSO WAVE.
 *  Bundled browser build for halo-ai.studio (single-file IIFE, exposes window.QRCode).
 *  Renders into a <canvas> via QRCode.render(canvas, text, opts).
 */
(function(global){
"use strict";
...
QRRSBlock.getRSBlocks = function(typeNumber, ecl){
    var rsBlock = QRRSBlock.getRsBlockTable(typeNumber, ecl);
    if (!rsBlock) throw new Error("bad rs block @ type:"+typeNumber+"/ecl:"+ecl);
    var length = rsBlock.length/3, list = [];
    for (var i=0;i<length;i++){
        var count = rsBlock[i*3], total = rsBlock[i*3+1], data = rsBlock[i*3+2];
        for (var j=0;j<count;j++) list.push(new QRRSBlock(total, data));
    }
    return list;
};
QRRSBlock.getRsBlockTable = function(typeNumber, ecl){
    switch(ecl){
...
function QR8bitByte(data){ this.mode = QRMode.MODE_8BIT_BYTE; this.data = data; this.parsedData = [];
    ...
    for (var i=0;i<this.data.length;i++){
        ...
        if (code > 0xffff){ // surrogate pair handled by charCodeAt; keep 8-bit fallback
...
QR8bitByte.prototype = {
    ...
    write: function(buf){ for (var i=0;i<this.parsedData.length;i++) buf.put(this.parsedData[i], 8); }
};

function QRCode(typeNumber, ecl){
    this.typeNumber = typeNumber;
    this.errorCorrectLevel = ecl;
    this.modules = null;
    this.moduleCount = 0;
    this.dataCache = null;
    this.dataList = [];
...
QRCode.prototype = {
    ...
    setupTypeInfo: function(test, maskPattern){
        ...
        var bits = QRUtil.getBCHTypeInfo(data), mod;
        for (var v=0;v<15;v++){
            mod = (!test && ((bits>>v)&1)===1);
...
QRCode.PAD0 = 0xEC;
QRCode.PAD1 = 0x11;
QRCode.createData = function(typeNumber, ecl, dataList){
...
QRCode.createBytes = function(buffer, rsBlocks){
    ...
    for (var r=0;r<rsBlocks.length;r++){
        ...
        for (var x=0;x<ecdata[r].length;x++){
            var idx = x + modPoly.getLength() - ecdata[r].length;
            ecdata[r][x] = (idx >= 0) ? modPoly.get(idx) : 0;
        }
    }
...
// Convenience: render into a <canvas>. opts: {size, margin, ecl, dark, light}.
QRCode.render = function(canvas, text, opts){
    opts = opts || {};
    var ecl = opts.ecl != null ? opts.ecl : QRErrorCorrectLevel.M;
    var qr = new QRCode(-1, ecl);
    qr.addData(text || "");
    qr.make();
    ...
    return { count: count, cell: cell, px: px };
};
