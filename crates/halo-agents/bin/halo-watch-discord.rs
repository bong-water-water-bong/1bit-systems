...
impl Handler {
    /// Single-line status pulled from halo-server /v1/models and
    /// halo-landing /metrics. Both probes are best-effort; a missing
...
// The heavy-lifting tests live in `halo_agents::watch::tests` so they run
// under `cargo test -p halo-agents`. The startup-without-token behaviour
