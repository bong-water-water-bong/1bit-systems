...
pub async fn run(url: Option<String>, stride: u32, max_tokens: u32, bytes: usize) -> Result<()> {
    let url = url.unwrap_or_else(|| std::env::var("HALO_PPL_URL").unwrap_or_else(|_| DEFAULT_URL.into()));
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
    let wikitext_path = std::env::var("HALO_WIKITEXT")
        .unwrap_or_else(|_| format!("{home}/halo-ai/datasets/wikitext-103-test.txt"));

    let text = std::fs::read_to_string(&wikitext_path)
        .with_context(|| format!("read {wikitext_path}"))?;
    ...
    println!("POST {url}  (stride={stride} max_tokens={max_tokens} {} bytes)", text.len());
    let res = client.post(&url).json(&body).send().await?;
    if !res.status().is_success() {
        bail!("ppl endpoint returned {}: {}", res.status(), res.text().await.unwrap_or_default());
    }
