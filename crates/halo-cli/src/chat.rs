...
pub async fn run(url: Option<String>, model: Option<String>, max_tokens: u32) -> Result<()> {
    let url = url.unwrap_or_else(|| std::env::var("HALO_CHAT_URL").unwrap_or_else(|_| DEFAULT_URL.into()));
    ...
    loop {
        ...
        if !res.status().is_success() {
            println!("  ← {} {}", res.status(), res.text().await.unwrap_or_default());
            continue;
        }
        let v: serde_json::Value = res.json().await?;
        let reply = v["choices"][0]["message"]["content"].as_str().unwrap_or("(empty)");
        let usage = &v["usage"];
        println!("{}", reply);
