...
fn resolve(short: &str) -> Option<&'static str> {
    SERVICES.iter().find(|(s, _, _)| *s == short).map(|(_, u, _)| *u)
}

pub async fn run(service: &str) -> Result<()> {
    let unit = resolve(service)
        .ok_or_else(|| anyhow::anyhow!("unknown service '{service}' (known: {})",
            SERVICES.iter().map(|(s, _, _)| *s).collect::<Vec<_>>().join(", ")))?;
