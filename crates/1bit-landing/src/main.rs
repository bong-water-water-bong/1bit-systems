//! `1bit-landing` — static marketing page + live status probe on :8190.
//!
...
//! `systemctl --user`. See `docs/wiki/Crate-1bit-landing.md` invariants.
//!
//! Routes:
//! * `GET /`              — embedded HTML
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("info,onebit_landing=debug")),
        )
        .init();
    ...
    info!(%addr, "1bit-landing listening");
...
mod tests {
    ...
    async fn index_serves_html_with_wordmark() {
        ...
        assert!(body.contains("1bit systems"), "missing wordmark in body");
    }
