...
//!
//! * Status  — 1bit-server reachability + loaded model + tok/s readout.
//! * Chat    — single input + assistant bubble list, hits
//!             `POST /v1/chat/completions` with `stream: true`.
//! * Skills  — `onebit_agents::SkillStore::list`.
//! * Memory  — `onebit_agents::MemoryStore::list(Memory)` + `list(User)`.
...
/// description. The underlying `onebit_agents::Skill` carries more; helm only
...
/// Snapshot of the 1bit-server's `/v1/models` list. Empty until refreshed.
...
impl HelmApp {
    ...
    pub fn refresh_skills(&mut self) {
        match onebit_agents::SkillStore::new().and_then(|s| s.list()) {
            Ok(skills) => {
                self.skills = skills
                    .into_iter()
                    .map(|s| SkillRow {
    ...
    pub fn refresh_memory(&mut self) {
        match onebit_agents::MemoryStore::new() {
            Ok(store) => {
...
mod tests {
    ...
    fn cfg() -> SessionConfig {
        SessionConfig::new("http://127.0.0.1:8180", "1bit-monster-2b")
    }
