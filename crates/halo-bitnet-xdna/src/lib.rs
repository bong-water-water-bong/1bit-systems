//! halo-bitnet-xdna — Rust wrapper around AMD XRT for dispatching kernels
...
pub enum XdnaError {
    ...
    #[error("halo-bitnet-xdna: built without real-xrt feature (stub mode)")]
    UnsupportedStub,

    /// The requested file (xclbin) does not exist on disk.
    #[error("halo-bitnet-xdna: file not found: {0}")]
    NotFound(String),

    /// Caller-side precondition violated (null pointer, mismatched lengths,
    /// bad UTF-8 path, etc.).
    #[error("halo-bitnet-xdna: invalid argument: {0}")]
...
impl XdnaDevice {
    ...
    pub fn open(idx: u32) -> Result<Self, XdnaError> {
        #[cfg(feature = "real-xrt")]
        {
            // SAFETY: halo_xrt_open either returns NULL or a valid pointer
            // to an opaque struct allocated by the C++ shim. The pointer
            // is owned by the returned XdnaDevice and freed in Drop.
            let raw = unsafe { ffi::halo_xrt_open(idx) };
            if raw.is_null() {
                tracing::warn!(bdf_idx = idx, "halo_xrt_open returned NULL");
    ...
    pub fn load_xclbin(&mut self, path: &Path) -> Result<(), XdnaError> {
        #[cfg(feature = "real-xrt")]
        {
            ...
            // SAFETY: self.raw is a non-null handle from halo_xrt_open;
            // path_cstr is a valid NUL-terminated C string that outlives
            // the call.
            let code = unsafe { ffi::halo_xrt_load_xclbin(self.raw, path_cstr.as_ptr()) };
    ...
    pub fn run_kernel(
        ...
        #[cfg(feature = "real-xrt")]
        {
            ...
            // SAFETY:
            //  - self.raw is a non-null handle from halo_xrt_open
...
impl Drop for XdnaDevice {
    fn drop(&mut self) {
        ...
        {
            if !self.raw.is_null() {
                // SAFETY: self.raw was produced by halo_xrt_open and has
                // not been freed yet. halo_xrt_close is NUL-safe anyway.
                unsafe { ffi::halo_xrt_close(self.raw) };
...
mod tests {
    ...
    /// Gated on NOT real-xrt so `cargo test -p halo-bitnet-xdna` on a real
