...
//
// We expose exactly what halo-bitnet-xdna's Rust side needs: open a device
...
//
// Lifetime contract:
//   - `halo_xrt_open` allocates an opaque device handle. Caller MUST
//     eventually free it with `halo_xrt_close`.
...
HaloXrtDevice* halo_xrt_open(uint32_t bdf_idx);

// Free a handle returned by halo_xrt_open. No-op on NULL.
void halo_xrt_close(HaloXrtDevice* dev);
...
int32_t halo_xrt_load_xclbin(HaloXrtDevice* dev, const char* path);
...
// up halo-router's NPU fallback path today.
//
