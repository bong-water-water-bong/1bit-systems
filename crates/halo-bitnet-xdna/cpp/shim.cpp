...
extern "C" HaloXrtDevice* halo_xrt_open(uint32_t bdf_idx) {
    try {
        auto* d = new HaloXrtDevice();
        d->device = xrt::device(bdf_idx);
        return d;
    } catch (const std::exception& e) {
        std::fprintf(stderr, "halo_xrt_open(%u) failed: %s\n", bdf_idx, e.what());
        return nullptr;
    } catch (...) {
        std::fprintf(stderr, "halo_xrt_open(%u) failed: unknown exception\n", bdf_idx);
        return nullptr;
    }
}

extern "C" void halo_xrt_close(HaloXrtDevice* dev) {
...
extern "C" int32_t halo_xrt_load_xclbin(HaloXrtDevice* dev, const char* path) {
    ...
    try {
        ...
        std::fprintf(stderr, "halo_xrt_load_xclbin(%s) failed: %s\n", path, e.what());
...
extern "C" int32_t halo_xrt_run_kernel(
    HaloXrtDevice* dev,
    const char*    name,
    const uint8_t* in,
    size_t         in_len,
    ...
    size_t         out_len)
{
    ...
    try {
        ...
        std::fprintf(stderr, "halo_xrt_run_kernel(%s) failed: %s\n", name, e.what());
