//! build.rs — propagate halo-bitnet-hip's rpath to the final binary.
//!
...
//! us. We read the resolved rocm-cpp lib dir from halo-bitnet-hip via the
...
fn main() {
    ...
    // halo-bitnet-hip isn't pulled in at all and the env var will be
    // absent.
...
fn emit_rpath(dir: &str) {
    ...
    // `-bins` applies to the `halo-server` binary target.
