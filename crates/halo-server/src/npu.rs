...
//!
//! Mirrors the logic in `halo-cli`'s `halo npu status` subcommand but returns
...
//! # Design notes
//!
//! * **Stub-mode safe.** We do NOT link `halo-bitnet-xdna` here — that crate
...
//!   `"stub"` in current halo-server builds because we don't link xdna at
//!   all).
...
/// Compile-time backend identity. Flipped to `"real-xrt"` if halo-server is
/// ever built with a live XDNA feature; today it's always `"stub"` because
/// halo-server doesn't link `halo-bitnet-xdna`.
...
pub struct NpuStatus {
    /// `"stub"` or `"real-xrt"` — reflects halo-server's build flags, NOT
