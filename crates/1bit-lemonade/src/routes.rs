...
async fn lemonade_health() -> Json<Value> {
    Json(json!({ "status": "ok", "engine": "1bit-lemonade" }))
}
    ...
    let data: Vec<Value> = s
        ...
        .map(|id| {
            ...
            json!({
                "id": id,
                "object": "model",
                "owned_by": entry.backend.split('/').next().unwrap_or("1bit systems"),
...
/// so the `onebit_lemonade_upstream_up` gauge is fresh for this scrape.
...
mod tests {
    ...
    async fn lemonade_health_shape() {
        ...
        assert_eq!(v["engine"], "1bit-lemonade");
    }
    ...
    async fn models_lists_registered() {
        ...
        assert_eq!(v["data"][0]["id"], "1bit-monster-2b");
    }
    ...
    async fn chat_proxy_upstream_down_returns_bad_gateway() {
        ...
        let app = build(test_state());
        let body = r#"{"model":"1bit-monster-2b","messages":[{"role":"user","content":"hi"}]}"#;
        let res = app
            .oneshot(
                Request::builder()
                    .method("POST")
    ...
    async fn metrics_endpoint_serves_prometheus_text() {
        ...
        assert!(
            text.contains("# TYPE onebit_lemonade_requests_total counter"),
            "missing counter TYPE marker: {text}"
        );
        assert!(
            text.contains("# TYPE onebit_lemonade_request_seconds histogram"),
            "missing histogram TYPE marker: {text}"
        );
        assert!(
            text.contains("# TYPE onebit_lemonade_upstream_up gauge"),
    ...
    async fn dispatch_records_metrics_for_each_call() {
        ...
        let res = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .uri("/v1/chat/completions")
                    .header("Content-Type", "application/json")
                    .body(Body::from(r#"{"model":"1bit-monster-2b","messages":[]}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        assert!(
            text.contains(
                "onebit_lemonade_requests_total{route=\"/v1/chat/completions\",status=\"200\"} 1"
            ),
            "expected counter line for successful POST, got:\n{text}"
        );
        assert!(
            text.contains("onebit_lemonade_upstream_up 1"),
