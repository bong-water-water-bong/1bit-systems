...
//!    (run this model locally on a halo-router backend, or proxy to some
...
/// mapping, etc. We keep this as [`anyhow::Error`] on purpose: halo-lemonade
...
pub trait Upstream: Send + Sync + std::fmt::Debug {
    /// Human-readable name for metrics labels + log lines (`"halo-server"`,
    /// `"lemond"`, `"flm"`, `"mock"`).
    fn name(&self) -> &str;
    ...
    /// Used by the `halo_lemonade_upstream_up` gauge.
    async fn health(&self) -> bool;
}

/// The concrete upstream we ship today: halo-server on 127.0.0.1:8180.
#[derive(Debug, Clone)]
...
impl Upstream for HaloServer {
    ...
    async fn health(&self) -> bool {
        // halo-server exposes /_health on the same port. We deliberately
        // use a short timeout — this probe runs on the /metrics path so
        // slow halo-server response can't block a Prometheus scrape.
...
mod tests {
    ...
    #[test]
    fn halo_server_normalises_trailing_slashes() {
    ...
    #[test]
    fn halo_server_rejects_garbage_url() {
