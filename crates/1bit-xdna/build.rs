//! build.rs — wire `1bit-xdna` to XRT (libxrt_coreutil + libxrt_core)
...
fn main() {
    ...
    if !real_xrt {
        println!(
            "cargo:warning=1bit-xdna built without `real-xrt` — \
    ...
    let xrt_include = find_xrt_include().expect(
        "1bit-xdna: real-xrt feature enabled but XRT headers not found. \
    ...
    let xrt_libdir = find_xrt_libdir().expect(
        "1bit-xdna: real-xrt feature enabled but libxrt_coreutil.so \
