...
//!   `is_ternary = false`). Example: a Q4 GGUF that halo-router loaded
...
//!   HTTP server. This is the **production dispatch shape**: halo-router
...
//!   * When FLM's output isn't re-serialisable into halo-server's KV
...
//!
//! FLM returns completion tokens, not a serialised KV cache. halo-server
...
/// halo-server's decode path. **Empty today** — see the module docstring;
    ...
    #[cfg(not(feature = "flm-subprocess"))]
    {
        ...
        return Err(BackendError::NotYetWired(
            "NPU prefill backend not loaded — flm-subprocess feature is off, \
             rebuild halo-router with --features flm-subprocess or set \
             HALO_BACKEND=hip",
        ));
    }
...
fn real_flm_prefill(
    ...
    Err(BackendError::NotYetWired(
        "FLM alive but KV handoff not implemented — flm serve → halo-server \
...
///
/// Not load-bearing — the router passes whatever model id halo-server
...
pub fn default_flm_model_id() -> String {
    ...
    // until halo-server plumbs its own catalog through.
...
mod tests {
    ...
    fn ternary_model_returns_npu_ternary_unsupported() {
        ...
        let err = flm_prefill_with_binary("hello", "halo-1bit-2b", true, fake)
