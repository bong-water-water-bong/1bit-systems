//! `VoicePipeline` — orchestrator: halo-server SSE → [`SentenceSplitter`]
...
//! 1. `halo-voice` CLI — pipes audio to `paplay`/`aplay` as chunks arrive.
//! 2. Future `halo-echo` crate — frames Opus packets over WebSocket to a
//!    browser peer.
//!
...
pub struct VoiceConfig {
    /// halo-server base URL (must expose `/v1/chat/completions`).
    pub llm_url: String,
