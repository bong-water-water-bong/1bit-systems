//! halo-voice — CLI that takes a prompt on stdin, streams audio
//! chunks to stdout as they arrive. Pipe into `paplay` / `aplay` /
//! `ffplay` for the speakers.
//!
//! ```bash
//! echo "tell me a one-sentence joke" | halo-voice --prompt - | paplay --raw
//! ```
//!
//! Or dump to disk:
//!
//! ```bash
//! halo-voice --prompt "explain rust" --out /tmp/reply.wav
//! ```
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "halo_voice=info".into()),
        )
        .with_target(false)
        .init();
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if cli.timing {
            eprintln!(
                "[halo-voice] chunk {} ({:>4} ms since start, {} bytes wav) sentence=\"{}\"",
                chunk.index,
    ...
    if cli.timing {
        eprintln!(
            "[halo-voice] done · {} chunks · {} bytes · {:.2} s wall",
            n_chunks,
            total_bytes,
