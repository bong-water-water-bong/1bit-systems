//! 1bit-mcp — JSON-RPC 2.0 stdio MCP server exposing `onebit_agents`
//! specialists as tools.
//!
//! Phase 1 wiring: `tools/call` dispatches through
//! [`onebit_agents::Registry`]. The tool list is derived from
//! [`onebit_agents::Name::ALL`] so the MCP-visible surface and the agents
//! bus are the same single source of truth.
//!
//! Entry points:
//!   * [`StdioServer::with_default_agents`] — prebuilt server with the
//!     default 17-stub `onebit_agents::Registry`.
