//! Tool registry — one MCP tool per `onebit_agents::Name` variant.
//!
//! The registry is a thin view over `onebit_agents::Name::ALL`. Tool names
//! are the bare specialist name (`anvil`, `muse`, …) so `tools/call` can
//! route directly into `onebit_agents::Registry::dispatch` with no rename
//! gymnastics.
//!
//! # Description + schema sourcing
//!
//! Starting with the `TypedSpecialist` work, descriptions and input
//! schemas are pulled from the live `onebit_agents::Registry` whenever one
...
///
/// In Phase 1 the tool name *is* the `onebit_agents` specialist name — the
...
impl Tool {
    ...
    /// `onebit_agents::Registry` for its description + input schema.
    ///
...
/// Ordered tool registry. Order matches `onebit_agents::Name::ALL` so clients
...
impl ToolRegistry {
    ...
    /// `onebit_agents::Registry` is available — typed specialists only
