...
impl TernaryBackend for MlxBackend {
    fn name(&self) -> &'static str { "mlx-apple (native, via bitnet-mlx-rs)" }
    fn supports_shape(&self, _: usize, _: usize) -> bool { true }
    ...
    fn generate(&mut self, _prompt: &[u32], _max_new: usize) -> anyhow::Result<Vec<u32>> {
        if !self.loaded { anyhow::bail!("call load_h1b() first"); }
        // TODO: bitnet_inference::generate(...)
        anyhow::bail!("mlx-apple generate: not wired yet")
    }
}

pub fn status() -> BackendStatus { BackendStatus::Ok }
