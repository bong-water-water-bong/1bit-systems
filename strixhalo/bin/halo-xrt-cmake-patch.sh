#!/usr/bin/env bash
# halo-ai XRT cmake patch — replaces CachyOS's broken xrt-targets.cmake
...
# projects (mlir-aie, hsa-runtime, our halo-bitnet-xdna when used with
...
cat > "$TARGET" <<'CMAKE'
# halo-ai patched xrt-targets.cmake — CachyOS xrt ships a broken file
# referencing static + opencl + pyxrt targets whose libraries aren't in
# the package. Replaced by /usr/local/sbin/halo-xrt-cmake-patch.sh
...
# halo-ai patched — no per-configuration overrides for shared-only set.
CMAKE
