#!/usr/bin/env bash
# halo-beta-expire.sh — 10-day TTL enforcer for the halo-ai private beta.
#
...
#      revoked via halo-mesh-revoke.sh <handle>.
...
#   - strixhalo/bin/halo-mesh-revoke.sh (reused as-is)
#   - strixhalo/bin/halo-mesh-invite.sh (issues the `expires` field)
...
REVOKE_CMD="${HALO_BETA_REVOKE_CMD:-$(dirname "$(readlink -f "$0")")/halo-mesh-revoke.sh}"
...
sweep_bearers() {
    ...
    # Revoke each handle. halo-mesh-revoke.sh drops the bearer + expires
