#!/usr/bin/env bash
# 1bit-mesh-invite.sh — generate Headscale pre-auth key + per-user bearer
# token for the 1bit systems 10-seat private beta.
#
# Usage:
...
#
# Revoke:
#   strixhalo/bin/1bit-mesh-revoke.sh <handle>      # expires both
#
...
# enforced by strixhalo/bin/1bit-beta-expire.sh (timer-driven).
...
╔══════════════════════════════════════════════════════════════╗
║ 1bit systems mesh invite — paste below to @$handle                 ║
╚══════════════════════════════════════════════════════════════╝

Welcome to the 1bit systems 10-seat private beta.
...
One-time steps (Linux / macOS):
  ...
  # 2. Join the 1bit systems mesh
  sudo tailscale up \\
       --login-server https://headscale.1bit.systems \\
       --authkey $authkey
  ...
  curl https://strixhalo.local/v2/v1/chat/completions \\
       ...
       -d '{"model":"1bit-monster-2b","messages":[{"role":"user","content":"hi"}]}'

Mobile (Tailscale iOS / Android): in the app, add a custom
coordination server https://headscale.1bit.systems and paste the
same --authkey.
...
ask bcloud to re-run 1bit-mesh-invite.sh for a fresh 10-day token.

Revoke on our side: strixhalo/bin/1bit-mesh-revoke.sh $handle
