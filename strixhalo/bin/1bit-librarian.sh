...
# Walks git log across 1bit systems-core + rocm-cpp + agent-cpp + halo-1bit,
# extracts conventional-commit-formatted lines since last run, appends
# to 1bit systems-core/CHANGELOG.md. Posts summary to Discord as 📚 librarian.

set -euo pipefail

STATE_FILE="$HOME/.local/share/1bit systems/librarian.state"
CHANGELOG="$HOME/1bit systems-core/CHANGELOG.md"
CHANNEL_ID="1488836467039010936"   # #changelog
MCP_WRAPPER="$HOME/repos/1bit systems-core/discord-mcp/bin/halo-discord.sh"

REPOS=(
    "$HOME/1bit systems-core:1bit systems-core"
...
if [[ ! -f "$CHANGELOG" ]]; then
    cat > "$CHANGELOG" <<EOF
# CHANGELOG — 1bit systems

Auto-appended by halo-librarian. Walks commit history of 1bit systems-core,
