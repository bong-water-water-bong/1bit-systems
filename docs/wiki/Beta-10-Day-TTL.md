...
**Policy (2026-04-20):** the 1bit systems private beta demo window is ten
...
2. **Demo window, not lifetime access.** The first ten days are a
   ...
   the invitee asks for a renewal, we re-run `1bit-mesh-invite.sh`,
...
| Headscale user + registered nodes | Headscale DB | `1bit-mesh-revoke.sh <handle>` (chained from the sweep) |
...
- **Load-bearing project memory files** — hard allow-list in the
  ...
  `strixhalo/bin/1bit-beta-expire.sh` and to the list above.
...
Just re-invite. `1bit-mesh-invite.sh` issues a fresh ten-day token and
...
After 2026-04-20, every line written by `1bit-mesh-invite.sh` looks
like:
...
- `strixhalo/bin/1bit-beta-expire.sh` — the sweep script
- `strixhalo/bin/1bit-mesh-invite.sh` — issuer (stamps `expires`)
- `strixhalo/bin/1bit-mesh-revoke.sh` — revoker (called per-handle)
