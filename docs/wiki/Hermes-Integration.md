...
When prompted for the endpoint, use the 1bit-proxy URL from the mesh:

```
base_url:    http://100.64.0.1:13306/v1          # Headscale mesh, strixhalo
model:       qwen3:1.7b                           # or any model exposed by /v1/models
api_key:     local-no-auth                        # local placeholder for clients that require a key
```

That's it. `hermes` TUI now drives `1bit-proxy`, which defaults to Lemonade and routes targeted FLM model families to FastFlowLM on the XDNA NPU lane. No detour through NousPortal/OpenRouter.
...
Hermes' 40+ built-in tools + our 17 1bit-agents specialists → 57-tool agent on commodity Python, talking to the local 1bit inference endpoint. Rule A untouched: Python lives on the laptop or peer, the core runtime stays on strixhalo.
...
| Three API modes (chat_completions / codex_responses / anthropic_messages) | Hermes' `runtime_provider.py` | **Already done** | `1bit-proxy` is OpenAI-compatible; we're the endpoint, not the caller. |
...
1. **Today**: wire Hermes → `1bit-proxy :13306` + expose 1bit-mcp (5-min operator setup, above).
...
- Local Lemonade + FastFlowLM inference behind one OpenAI-compatible endpoint (Hermes is model-agnostic — the model is whatever endpoint you point it at)
