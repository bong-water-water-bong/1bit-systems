...
boots, answer DMs on Discord/Telegram/web, and use `lemond:8180` —
our own ternary BitNet engine — as their brain. No API keys, no rate
limits, no Claude.
...
- **Brain** — POSTs OpenAI-compat `/v1/chat/completions` at
  `lemond` on `127.0.0.1:8180`. Runs our 1.58-bit model on the
  iGPU. Same brain you'd use from Open WebUI.
...
`After=1bit-halo-lemonade.service` mean the agent comes back after
...
- **Not a Claude wrapper.** The brain is `lemond`. No cloud calls
  on the runtime path.
...
brain (`lemond`), the kernels (`rocm-cpp`), the tools (`mcp`,
`retrieval`), and the engine (`core`) all sit underneath. The agent
