# Sorana → halo-server integration

Research date: 2026-04-19. Scope: pointing the Sorana Visual AI Workspace
(https://tetramatrix.github.io/Sorana/) at our local `halo-server`
(127.0.0.1:8180, OpenAI-compat) without installing Sorana on this box.

## Summary

**Sorana does not work out-of-the-box with halo-server.** Three blockers
...
2. **Sorana's backend layer is Lemonade-first.** The sister project
   ...
   `/api/v1/health`. Our halo-server is on **:8180** and serves `/v1/*`
...
schema Sorana's SDK expects is the same schema halo-server already
...
exists**. For a halo-ai user today, Sorana is not a reachable frontend.
...
- **(c) Sorana bundles its own inference.** Partially true — the
  ...
  all if they choose. For halo-ai users who want their strix halo to
...
discovers first). Forward everything to halo-server.
...
Same shape as the existing `halo-lemonade` crate — in fact we should
likely **fold this into `halo-lemonade`** rather than spawn a new
crate. `halo-lemonade` already binds a gateway port and already has the
"proxy to halo-server" intent documented in its `lib.rs`. Add the
...
halo-server behind Caddy at `https://strixhalo.local/v2/chat/completions`
requires `Authorization: Bearer sk-halo-...`. The **local** halo-server
on `127.0.0.1:8180` does not — Caddy is the bearer gate. Two options:

- **Local mode (recommended).** Run the shim alongside halo-server on
...
- **Streaming (SSE).** halo-server emits `text/event-stream` when
  ...
  streaming too.
- **Function calling / tools.** halo-server's tool-call surface is
  currently a TODO per `crates/halo-server/src/routes.rs`. Sorana
  advertises "no-code agent pipelines" which likely rely on tool
  calls. Expect agent-builder workflows to misbehave until halo-server
  gets real tool dispatch.
- **MCP.** Sorana does not advertise MCP-client support. Our
  `halo-mcp` bridge is not relevant to this integration.
- **Embeddings / audio / image.** The Lemonade SDK mentions embeddings,
  TTS, STT, rerank, and image endpoints. halo-server only serves
...
- **Model list.** halo-server reports whatever `halo-router` has
...
2. If a user *insists* on a polished desktop workspace pointed at
   halo-server, recommend **open-webui** or **LibreChat** instead.
...
If the shim-in-`halo-lemonade` work lands anyway (because we wanted it
...
- halo-server routes — `crates/halo-server/src/routes.rs` (OpenAI
  `/v1/*` paths, no `/api/v1` prefix, bound on `:8180`)
- halo-lemonade scaffold — `crates/halo-lemonade/src/lib.rs` (future
